...
class MockStore:
    ...
    def create_order(self, payload):
        with self.lock:
            ...
            menu = self.data.setdefault("menus", {}).get(
            ...
            for it in payload.get("items", []):
                ...
                items_out.append(
                    {
                        "item_id": mi["id"],
                        "nombre": mi.get("nombre"),
                        "precio": mi.get("precio"),
    ...
    services = [
        {
            "name": "Autenticación",
            "key": "auth",
            "health": f"{API_GATEWAY_URL}/api/v1/auth/health",
        },
        {
            "name": "Restaurantes",
            "key": "restaurantes",
            "health": f"{API_GATEWAY_URL}/api/v1/restaurantes/health",
        },
        {
            "name": "Pedidos",
            "key": "pedidos",
            "health": f"{API_GATEWAY_URL}/api/v1/pedidos/health",
        },
        {
            "name": "Repartidores",
            "key": "repartidores",
            "health": f"{API_GATEWAY_URL}/api/v1/repartidores/health",
        },
    ]
    ...
    if request.method == "POST":
        ...
        try:
            try:
                ...
                print(
                    "[FRONTEND][LOGIN] Gateway timeout, using direct auth service",
                    flush=True,
                )
                resp = requests.post(
            ...
            if resp.status_code == 200:
                ...
                if token:
                    ...
                    try:
                        try:
                            me = requests.get(
                                f"{API_GATEWAY_URL}/api/v1/auth/me",
                                headers={"Authorization": f"Bearer {token}"},
                                timeout=2,
                            )
                        except requests.exceptions.RequestException:
                            print(
                                "[FRONTEND][LOGIN] Gateway /me timeout, using direct auth service",
                                flush=True,
                            )
                            me = requests.get(
                                "http://authentication:8001/me",
                                headers={"Authorization": f"Bearer {token}"},
                                timeout=5,
                            )
                        ...
                        if me.status_code == 200:
                            ...
                            if role == "admin":
                                flash(
                                    "El ingreso como administrador no está permitido desde esta interfaz."
                                )
                                return render_template("login.html", title="Login")
        ...
        try:
            ...
            if resp.status_code not in (200, 201):
                ...
                direct = requests.post(
                    "http://restaurantes-service:8002/api/v1/restaurantes/",
                    json=rest_payload,
                    timeout=4,
                    allow_redirects=True,
                )
                print(
                    f"[RESTAURANT] Direct service response: {direct.status_code}",
                    flush=True,
                )
                if direct.status_code not in (200, 201):
                    flash(
                        f"No se pudo crear el restaurante. Status: {direct.status_code}"
                    )
                    print(
                        f"[RESTAURANT] Failed to create restaurant: {direct.text}",
                        flush=True,
                    )
                    return render_template(
                        "new_restaurant.html", title="Registrar restaurante"
                    )
                resto = direct.json()
            else:
                resto = resp.json()
                ...
                try:
                    files = {
                        "file": (
                            foto.filename,
                            foto.stream.read(),
                            foto.mimetype or "application/octet-stream",
                        )
                    }
                    try:
                        r = requests.post(
                            f"{API_GATEWAY_URL}/api/v1/restaurantes/{rest_id}/photo",
                            files=files,
                            headers=headers,
                            timeout=5,
                        )
                        if r.status_code not in (200, 201):
                            # fallback direct
                            requests.post(
                                f"http://restaurantes-service:8002/api/v1/restaurantes/{rest_id}/photo",
                                files=files,
                                timeout=4,
                            )
                    except requests.exceptions.RequestException:
                    ...
                    flash(
                        "Advertencia: no se pudo subir la foto, puedes añadirla después."
                    )
            ...
            for i, nombre_item in enumerate(nombres or []):
                ...
                try:
                    r = requests.post(
                        f"{API_GATEWAY_URL}/api/v1/restaurantes/{rest_id}/menu/",
                        json=item_payload,
                        headers=headers,
                        timeout=4,
                        allow_redirects=True,
                    ...
                    if r.status_code not in (200, 201):
                        # fallback
                        requests.post(
                            f"http://restaurantes-service:8002/api/v1/restaurantes/{rest_id}/menu/",
                            json=item_payload,
                            timeout=4,
                            allow_redirects=True,
                        )
            ...
            print(
                f"[RESTAURANT] RequestException: {type(e).__name__}: {str(e)}",
                flush=True,
            )
            ...
            print(
                f"[RESTAURANT] Unexpected error: {type(e).__name__}: {str(e)}",
                flush=True,
            )
...
def restaurant_dashboard():
    ...
    if user_id and not restaurant_id:
        try:
            try:
                resp = requests.get(
                    f"{API_GATEWAY_URL}/api/v1/restaurantes/by-user/{user_id}",
                    headers=headers,
                    timeout=3,
                )
            except requests.exceptions.RequestException:
                resp = requests.get(
                    f"http://restaurantes-service:8002/api/v1/restaurantes/by-user/{user_id}",
                    timeout=3,
                )
            ...
            if resp.status_code == 200:
                ...
                session["restaurant_id"] = (
                    restaurant_id  # Save to session for future requests
                )
        except Exception:
            pass
    ...
    if restaurant_id:
        try:
            ...
            if not restaurante:
                try:
                    resp = requests.get(
                        f"{API_GATEWAY_URL}/api/v1/restaurantes/{restaurant_id}",
                        headers=headers,
                        timeout=3,
                    )
                except requests.exceptions.RequestException:
                    resp = requests.get(
                        f"http://restaurantes-service:8002/api/v1/restaurantes/{restaurant_id}",
                        timeout=3,
                    )
    ...
    if restaurant_id:
        try:
            ...
            print(
                f"[DASHBOARD] Loading orders for restaurant_id={restaurant_id}, year={year}, month={month}",
                flush=True,
            )
            ...
            if orders_resp and orders_resp.status_code == 200:
                ...
                print(
                    f"[DASHBOARD] Loaded {len(orders_data.get('orders', []))} orders",
                    flush=True,
                )
            elif orders_resp:
                print(
                    f"[DASHBOARD] Orders response status: {orders_resp.status_code}",
                    flush=True,
                )
        except Exception as e:
...
def add_menu_items():
    ...
    # GET request
    return render_template(
        "add_menu_items.html", title="Agregar Items al Menú", restaurante=restaurante
    )
...
def delete_menu_item(restaurant_id: str, item_id: str):
    ...
    if session.get("restaurant_id") != restaurant_id:
        return jsonify(
            {"success": False, "error": "No autorizado para este restaurante"}
        ), 403
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            ...
            return jsonify(
                {"success": False, "error": f"Error del servidor: {resp.status_code}"}
            ), resp.status_code
...
def repartidor_setup():
    ...
    if not user_id:
        try:
            me = requests.get(
                f"{API_GATEWAY_URL}/api/v1/auth/me", headers=headers, timeout=4
            )
            if me.status_code == 200:
                u = me.json()
                if isinstance(u, dict) and "user" in u:
    ...
    if request.method == "GET":
        try:
            resp = requests.get(
                f"{API_GATEWAY_URL}/api/v1/repartidores/{user_id}",
                headers=headers,
                timeout=3,
            )
            if resp.status_code == 200:
                ...
                try:
                    resp = requests.get(
                        f"http://repartidores-service:8004/api/v1/repartidores/{user_id}",
                        timeout=3,
                    )
                    if resp.status_code == 200:
                        existing_repartidor = resp.json()
    ...
    if request.method == "POST":
        ...
        if not nombre:
            ...
            return render_template(
                ...
                repartidor=existing_repartidor,
            )
        ...
        if repartidor_exists:
            ...
            try:
                resp = requests.post(
                    f"{API_GATEWAY_URL}/api/v1/repartidores",
                    json=payload,
                    headers=headers,
                    timeout=5,
                )
                if resp.status_code not in (200, 201):
                    direct = requests.post(
                        "http://repartidores-service:8004/api/v1/repartidores",
                        json=payload,
                        timeout=4,
                    )
                    if direct.status_code not in (200, 201):
        ...
        if foto and foto.filename:
            try:
                ...
                try:
                    r = requests.post(
                        f"{API_GATEWAY_URL}/api/v1/repartidores/{user_id}/photo",
                        files=files,
                        headers=headers,
                        timeout=5,
                    )
                    if r.status_code not in (200, 201):
                        requests.post(
                            f"http://repartidores-service:8004/api/v1/repartidores/{user_id}/photo",
                            files=files,
                            timeout=4,
                        )
                except requests.exceptions.RequestException:
        ...
        flash(
            ...
            if repartidor_exists
            else "Registro de repartidor completado."
        )
    ...
    return render_template(
        "new_repartidor.html",
        title="Actualizar perfil" if existing_repartidor else "Registro repartidor",
        repartidor=existing_repartidor,
    )
...
def new_user():
    ...
    role_default = (
        ...
        if request.method == "GET"
        else (request.form.get("role") or request.args.get("role"))
    )
    ...
    if request.method == "POST":
        ...
        print(
            f"[FRONTEND][REGISTER] Form data - email={email}, role={role}", flush=True
        )

        # Do not allow creating admin users via the frontend
        if role == "admin":
            flash(
                "No está permitido crear usuarios con rol administrador desde esta interfaz."
            )
            return render_template(
                "new_user.html", title="Nuevo Usuario", role_default=role_default
            )
        ...
        print(
            f"[FRONTEND][REGISTER] Attempting register for {email} role={role}",
            flush=True,
        )
        try:
            print(
                f"[FRONTEND][REGISTER] Calling gateway at {API_GATEWAY_URL}", flush=True
            )
            try:
                resp = requests.post(
                    f"{API_GATEWAY_URL}/api/v1/auth/register", json=payload, timeout=2
                )
                print(
                    f"[FRONTEND][REGISTER] Gateway register response status={resp.status_code}",
                    flush=True,
                )
            except requests.exceptions.RequestException as gw_ex:
                print(
                    f"[FRONTEND][REGISTER] Gateway failed ({gw_ex}), trying direct auth service",
                    flush=True,
                )
                resp = requests.post(
                    "http://authentication:8001/register", json=payload, timeout=5
                )
                print(
                    f"[FRONTEND][REGISTER] Direct register response status={resp.status_code}",
                    flush=True,
                )
            ...
            print(
                f"[FRONTEND][REGISTER] Final resp.status_code={resp.status_code}",
                flush=True,
            )
            if resp.status_code in (200, 201):
                # Autologin: intentar iniciar sesión inmediatamente después del registro
                print(
                    f"[FRONTEND][REGISTER] Register success, attempting autologin for {email} role={role}",
                    flush=True,
                )
                ...
                try:
                    try:
                        ...
                        print(
                            "[FRONTEND][REGISTER] Gateway login timeout, using direct auth service",
                            flush=True,
                        )
                        login_resp = requests.post(
                            "http://authentication:8001/login",
                        ...
                        if token:
                            ...
                            try:
                                try:
                                    ...
                                    print(
                                        "[FRONTEND][REGISTER] Gateway /me timeout, using direct auth service",
                                        flush=True,
                                    )
                                    me = requests.get(
                                        "http://authentication:8001/me",
                                ...
                                if me.status_code == 200:
                                    ...
                                    if u.get("role") == "admin":
                                        flash(
                                            "No está permitido iniciar sesión como admin desde esta interfaz."
                                        )
                                        return redirect(url_for("login"))
                                    session["access_token"] = token
                                    session["user_email"] = u.get("email")
                                    ...
                                    print(
                                        f"[FRONTEND][REGISTER] Autologin success for {u.get('email')} role={u.get('role')}",
                                        flush=True,
                                    )
                                else:
                                    print(
                                        f"[FRONTEND][REGISTER] /me failed status={me.status_code}",
                                        flush=True,
                                    )
                            except requests.exceptions.RequestException as e:
                                print(
                                    f"[FRONTEND][REGISTER] /me request exception: {e}",
                                    flush=True,
                                )
                        else:
                            print(
                                "[FRONTEND][REGISTER] No access_token in login response",
                                flush=True,
                            )
                    else:
                        print(
                            f"[FRONTEND][REGISTER] Login failed status={login_resp.status_code}",
                            flush=True,
                        )
                except requests.exceptions.RequestException as e:
                    print(
                        f"[FRONTEND][REGISTER] Login request exception: {e}", flush=True
                    )
                ...
                if autologin_success:
                    ...
                    if (
                        role == "restaurante"
                        or session.get("user_role") == "restaurante"
                    ):
                        return redirect(url_for("restaurant_setup"))
                    return redirect(url_for("client_index"))
            ...
            print(
                f"[FRONTEND][REGISTER] RequestException in outer try-except: {e}",
                flush=True,
            )
            flash(f"Error conectando al gateway: {e}")
        except Exception as e:
            print(
                f"[FRONTEND][REGISTER] Unexpected exception: {type(e).__name__}: {e}",
                flush=True,
            )
...
def client_index():
    ...
    return render_template(
        "client_home.html", title="Buscar restaurantes", restaurantes=restos, q=q
    )
...
def restaurant_detail(rest_id):
            ...
            if r.status_code == 404:
                try:
                    direct_r = requests.get(
                        f"http://restaurantes-service:8002/api/v1/restaurantes/{rest_id}",
                        timeout=5,
                    )
    ...
    try:
        m = requests.get(
            f"{API_GATEWAY_URL}/api/v1/restaurantes/{rest_id}/menu", timeout=5
        )
    ...
    if request.method == "POST":
        ...
        try:
            resp = requests.post(
                f"{API_GATEWAY_URL}/api/v1/pedidos",
                json=payload,
                headers={"Authorization": f"Bearer {token}"},
                timeout=5,
            )
            if resp.status_code in (200, 201):
                order = resp.json()
                order_id = (
                    order.get("id") or order.get("pedido_id") or order.get("order_id")
                )
                try:
                ...
                if resp.status_code == 404:
                    try:
                        direct = requests.post(
                            "http://pedidos-service:8003/api/v1/pedidos",
                            json=payload,
                            headers={"Authorization": f"Bearer {token}"},
                            timeout=5,
                        )
                        if direct.status_code in (200, 201):
                            ...
                            msg = (
                                direct.json().get("detail")
                                if direct.headers.get("content-type", "").startswith(
                                    "application/json"
                                )
                                else direct.text
                            )
                        ...
                        flash(
                            "No se pudo conectar al servicio de pedidos directamente."
                        )
                else:
                    msg = (
                        resp.json().get("detail")
    ...
    if order_id:
        try:
            ...
            resp = requests.get(
                f"{API_GATEWAY_URL}/api/v1/pedidos/{order_id}",
                headers=headers,
                timeout=3,
            )
            if resp.status_code == 200:
    ...
    return render_template(
        "restaurant.html",
        title=restaurante.get("nombre", "Restaurante"),
        restaurante=restaurante,
        menu=menu,
...
def order_status(order_id):
    ...
    try:
        try:
            ...
            resp = requests.get(
                f"http://pedidos-service:8003/api/v1/pedidos/{order_id}",
                headers=headers,
                timeout=5,
            )
    ...
    return render_template(
        "order_confirm.html", title="Estado del pedido", pedido=pedido
    )
...
def api_rest_menu(rest_id):
    ...
    try:
        resp = requests.get(
            f"{API_GATEWAY_URL}/api/v1/restaurantes/{rest_id}/menu", timeout=4
        )
        if resp.status_code == 200:
            return (
                resp.content,
            ...
            if resp.status_code == 404:
                try:
                    ...
                    return (
                        ...
                        direct.status_code,
                        {
                            "Content-Type": direct.headers.get(
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            ...
            if resp.status_code == 405 or (
                resp.status_code in (404, 401) and not token
            ):
                try:
                    direct = requests.post(
                        "http://pedidos-service:8003/api/v1/pedidos",
        ...
        try:
            ...
            try:
                ...
                return (
                    {
                        "detail": "unable to create order (gateway and local mock failed)"
                    },
                    500,
                )
...
def repartidor_photo(rep_id):
    ...
    try:
        ...
        try:
            resp = requests.get(
                f"{API_GATEWAY_URL}/api/v1/repartidores/{rep_id}/photo",
                timeout=1,
                stream=True,
            )
            if resp.status_code == 200:
        ...
        direct = requests.get(
            f"http://repartidores-service:8004/api/v1/repartidores/{rep_id}/photo",
            timeout=3,
            stream=True,
        )
        if direct.status_code == 200:
            return (
                direct.content,
                direct.status_code,
                {"Content-Type": direct.headers.get("content-type", "image/jpeg")},
            )
        return ("", 404)
    except Exception:
        return ("", 404)
...
def repartidor_dashboard():
    ...
    try:
        resp = requests.get(
            f"{API_GATEWAY_URL}/api/v1/repartidor/{user_id}/orders?year={year}&month={month}",
            headers={"Authorization": f"Bearer {session.get('access_token')}"},
            timeout=5,
        )
        if resp.status_code != 200:
            # fallback direct
            resp = requests.get(
                f"http://pedidos-service:8003/api/v1/repartidor/{user_id}/orders?year={year}&month={month}",
                timeout=4,
            )
    except requests.exceptions.RequestException:
        try:
            ...
            flash(
                "No se pudo conectar al servicio de pedidos para obtener tus órdenes."
            )
            return render_template(
                "repartidor_dashboard.html",
                orders=[],
                current_order=None,
                gain_current=0.0,
                gain_others=0.0,
    ...
    try:
        ...
        if rep_resp.status_code == 200:
            ...
            if (
                repartidor_data.get("nombre")
                and repartidor_data.get("telefono")
                and repartidor_data.get("foto_url")
            ):
...
def completar_pedido_repartidor(order_id):
    ...
    try:
        try:
            resp = requests.post(
                f"{API_GATEWAY_URL}/api/v1/pedidos/{order_id}/complete",
                headers=headers,
                timeout=2,
            )
        except requests.exceptions.RequestException:
            resp = requests.post(
                f"http://pedidos-service:8003/api/v1/pedidos/{order_id}/complete",
                headers=headers,
                timeout=5,
            )
